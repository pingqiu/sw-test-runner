...
func kubectlWaitCondition(ctx context.Context, actx *tr.ActionContext, act tr.Action) (map[string]string, error) {
	...
	// Use \" inside JSONpath so the outer single-quoted shell wrapper doesn't
	// collapse against inner single quotes (which would yield invalid JSONpath
	// like ?(@.type==Available) without quoting → kubectl: unrecognized identifier).
